...
class ObsidianTool {
    ...
    async run(params) {
        const { action, query = '', path: filePath = '', limit = 10 } = params;
        if (!OBSIDIAN_VAULT_PATH) {
            throw new Error('OBSIDIAN_VAULT_PATH environment variable is not set');
        }
        const fullPath = path_1.default.join(OBSIDIAN_VAULT_PATH, filePath);
        try {
            switch (action) {
                case 'search':
            ...
            for (const file of files) {
                ...
                try {
                    ...
                    if (content.toLowerCase().includes(searchTerm)) {
                        const relativePath = path_1.default.relative(OBSIDIAN_VAULT_PATH, file);
                        results.push({
                            path: relativePath,
                            title: path_1.default.basename(file, '.md'),
                            excerpt: this.getExcerpt(content, searchTerm),
                            size: content.length,
                            modified: (await fs_extra_1.default.stat(file)).mtime
                        });
                    }
                }
                catch (error) {
    ...
    async readNote(filePath) {
        try {
            ...
            return {
                path: path_1.default.relative(OBSIDIAN_VAULT_PATH, filePath),
                title: path_1.default.basename(filePath, '.md'),
                content,
                size: content.length,
    ...
    async listNotes(directory, limit) {
        try {
            const fullPath = path_1.default.join(OBSIDIAN_VAULT_PATH, directory);
            ...
            return {
                ...
                files: files.slice(0, limit).map(file => ({
                    path: file.path,
                    name: path_1.default.basename(file.path, '.md'),
